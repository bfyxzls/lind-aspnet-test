...
namespace WEBAPI.Core
{
    ...
    public class RabbitMQPublisher
    {
        ...
        public RabbitMQPublisher(string uri = "amqp://localhost:5672", string userName = "guest",
            string password = "guest", string exchangeName = "")
        {
            _uri = uri;
        ...
        public void Publish<TMessage>(string exchange, string queue, TMessage message)
        {
            ...
            using (var channel = connection.CreateModel())
            {
                channel.QueueDeclare(queue, //队列名
                    true, //是否持久化
                    false, //排它性
                    false, //一旦客户端连接断开则自动删除queue
                    null); //如果安装了队列优先级插件则可以设置优先级

                var json = JsonHelper.SerializeObject(message);
                var bytes = Encoding.UTF8.GetBytes(json);
                Console.WriteLine("向服务器{0}推消息", _uri);
                channel.BasicPublish(exchangeName, queue, null, bytes);
            }
        }
        ...
        public void PublishFanout<TMessage>(string exchange, TMessage message)
        {
            ...
            using (var channel = connection.CreateModel())
            {
                ...
                var json = JsonHelper.SerializeObject(message);
                var bytes = Encoding.UTF8.GetBytes(json);
                channel.BasicPublish(exchangeName, ROUTING_KEY, null, bytes); //不需要指定routing key，设置了fanout,指了也没有用.
            ...
            using (var channel = connection.CreateModel())
            {
                ...
                foreach (var message in messages)
                    try
                    {
                        ...
                        batch.Add(exchangeName, queue, false, null, body);
